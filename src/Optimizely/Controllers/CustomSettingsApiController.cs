...
public class CustomSettingsApiController : ControllerBase
{
    ...
    public async Task<IActionResult> GetSettings(string groupId, [FromQuery] string? siteId, [FromQuery] string? language)
    {
        ...
        if (!string.IsNullOrEmpty(siteId))
        {
            ...
            if (_siteDefinitionRepository.Get(siteGuid) == null)
    ...
    public IActionResult SearchContent([FromQuery] string? q, [FromQuery] string? language, [FromQuery] string? type)
    {
        try
        {
            ...
            if (wantMedia)
            {
                ...
                {
                    if (!ContentReference.IsNullOrEmpty(site.GlobalAssetsRoot))
                        roots.Add(site.GlobalAssetsRoot);
                    if (!ContentReference.IsNullOrEmpty(site.SiteAssetsRoot) &&
                        site.SiteAssetsRoot != site.GlobalAssetsRoot)
                        roots.Add(site.SiteAssetsRoot);
                }
            }
            var content = roots
                .Distinct()
    ...
    private IActionResult? ValidateLanguage(string? language, Guid? siteId)
    {
        ...
        if (availableList.Count == 0 || !availableList.Contains(language, StringComparer.OrdinalIgnoreCase))
        {
